...
def build_feature_file() -> Path:
    ...
    output_path = ARTIFACTS_DIR / "content_features_metadata_source075_w15_with_category_profile_only.pt"
...
def main() -> None:
    ...
    gate_inits = [-3.0, -2.5, -2.0, -1.5]
    rows = []
    for gate_init in gate_inits:
        gate_slug = str(gate_init).replace("-", "m").replace(".", "p")
        experiment_name = f"beauty2013_metadata_v36_source075_w15_catprofileonly_{gate_slug}"
        ...
        config.category_profile_gate_init = float(gate_init)
        ...
        row = {
            ...
            "category_profile_gate_init": gate_init,
    ...
    output_path = Path("outputs/paper/beauty2013/beauty2013_category_profile_only_v36_summary.json")
