...
DEFAULT_FEATURE_PATH = ROOT_DIR / "artifacts" / "content_features_metadata_source075_w15_with_category_profile_only.pt"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run multi-seed follow-up for the profile-only category auxiliary variant.")
    parser.add_argument("--feature-path", default=str(DEFAULT_FEATURE_PATH))
    parser.add_argument("--category-profile-gate-init", type=float, default=-2.5)
    parser.add_argument("--experiment-tag", default="v39_profile_seed")
...
def main() -> None:
    ...
    rows = []
    for seed in seeds:
        experiment_name = f"beauty2013_metadata_{args.experiment_tag}_seed{seed}"
        ...
        config = MMTrainConfig(
            ...
            category_profile_gate_init=args.category_profile_gate_init,
        )
