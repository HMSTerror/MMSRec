...
def main() -> None:
    text_profile_gates = [-7.0, -6.0, -5.0, -4.0]
    rows = []
    for gate_init in text_profile_gates:
        gate_slug = str(gate_init).replace("-", "m").replace(".", "p")
        experiment_name = f"beauty2013_metadata_v45_source075_w15_catprofile_m2p5_textprofile_{gate_slug}"
        ...
        config = MMTrainConfig(
            ...
            text_profile_gate_init=gate_init,
    ...
    output_path = Path("outputs/paper/beauty2013/beauty2013_category_profile_textprofile_v45_summary.json")
