...
def build_feature_file() -> Path:
    ...
    output_path = ARTIFACTS_DIR / "content_features_metadata_text_category2x_with_category_branch.pt"
...
def main() -> None:
    ...
    gate_inits = [-1.5, -1.0, -0.5]
    rows = []
    for gate_init in gate_inits:
        gate_slug = str(gate_init).replace("-", "m").replace(".", "p")
        experiment_name = f"beauty2013_metadata_v34_text_category2x_catbranch_{gate_slug}"
        ...
        config = MMTrainConfig(
            ...
            category_score_gate_init=gate_init,
    ...
    output_path = Path("outputs/paper/beauty2013/beauty2013_category_branch_v34_summary.json")
